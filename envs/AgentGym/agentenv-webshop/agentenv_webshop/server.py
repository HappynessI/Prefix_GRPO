...
@app.post("/close", response_model=bool)
def close(close_query: dict):
    """Close a specific environment"""
    env_idx = close_query.get("env_idx")
    print(f"/close - env_idx: {env_idx}")
    result = webshop_env_server.close(env_idx)
    return result
