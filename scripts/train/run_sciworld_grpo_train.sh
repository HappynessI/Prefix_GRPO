...
DATA_PATH=${DATA_PATH:-"${DATA_ROOT}/sciworld/train.parquet"}
OUTPUT_DIR=${OUTPUT_DIR:-"${OUTPUT_ROOT}/sciworld_grpo"}
INTERACTION_CONFIG=${INTERACTION_CONFIG:-"${CONFIG_ROOT}/interaction_config/sciworld_interaction.yaml"}
...
WHEEL_DIR=${WHEEL_DIR:-"${PROJECT_ROOT}/third_party/wheels_sciworld"}
RUNTIME_REQS=${RUNTIME_REQS:-"${PROJECT_ROOT}/third_party/requirements_sciworld_runtime.txt"}
...
SCIWORLD_JAVA_HOME=${SCIWORLD_JAVA_HOME:-"${PROJECT_ROOT}/third_party/jre_sciworld"}
SCIWORLD_PORT=${SCIWORLD_PORT:-36003}
SCIWORLD_SERVER="http://127.0.0.1:${SCIWORLD_PORT}"
SCIWORLD_LOG=""
...
SCIWORLD_LOG="$LOG_DIR/sciworld_env_${TIMESTAMP}.log"
...
cleanup_sciworld() {
    if [ -n "$SCIWORLD_PID" ] && kill -0 "$SCIWORLD_PID" 2>/dev/null; then
        echo "" | tee -a "$LOG_FILE"
        echo "清理 SciWorld 环境服务 (PID=$SCIWORLD_PID)..." | tee -a "$LOG_FILE"
        kill "$SCIWORLD_PID" 2>/dev/null || true
        wait "$SCIWORLD_PID" 2>/dev/null || true
        echo "SciWorld 环境服务已清理。" | tee -a "$LOG_FILE"
    fi
}
trap cleanup_sciworld EXIT

configure_sciworld_java() {
    echo "配置 SciWorld Java 运行时..." | tee -a "$LOG_FILE"

    if [ -n "${JAVA_HOME:-}" ] && [ -x "${JAVA_HOME}/bin/java" ]; then
        export PATH="${JAVA_HOME}/bin:${PATH}"
        echo "  使用 JAVA_HOME=${JAVA_HOME}" | tee -a "$LOG_FILE"
    elif [ -x "${SCIWORLD_JAVA_HOME}/bin/java" ]; then
        export JAVA_HOME="${SCIWORLD_JAVA_HOME}"
        export PATH="${JAVA_HOME}/bin:${PATH}"
        echo "  使用项目内置 JRE: ${JAVA_HOME}" | tee -a "$LOG_FILE"
    elif command -v java >/dev/null 2>&1; then
        echo "  使用系统 java: $(command -v java)" | tee -a "$LOG_FILE"
    else
        echo "错误: 未找到 java，可执行文件是 ScienceWorld 运行时必需依赖。" | tee -a "$LOG_FILE"
        echo "请确认以下至少一项成立：" | tee -a "$LOG_FILE"
        echo "  1. 上传 ${PROJECT_ROOT}/third_party/jre_sciworld，并确保其中存在 bin/java" | tee -a "$LOG_FILE"
        echo "  2. 通过 JAVA_HOME 指向可用 JRE/JDK" | tee -a "$LOG_FILE"
        echo "  3. 镜像 PATH 中已包含 java" | tee -a "$LOG_FILE"
        exit 1
    fi
...
echo "============================================" | tee -a "$LOG_FILE"
echo "正在当前 Pod 内启动 SciWorld 环境服务" | tee -a "$LOG_FILE"
echo "============================================" | tee -a "$LOG_FILE"

echo "离线安装 SciWorld 运行时依赖..." | tee -a "$LOG_FILE"
...
if [ ! -d "$WHEEL_DIR" ]; then
    ...
    echo "请在开发机运行 scripts/prepare_sciworld_wheels.sh 并重新上传代码。" | tee -a "$LOG_FILE"
    exit 1
fi
...
if ! python3 -m pip install \
        ...
        -r "$RUNTIME_REQS" \
        2>&1 | tee -a "$LOG_FILE"; then
    echo "错误: SciWorld 运行时依赖离线安装失败。" | tee -a "$LOG_FILE"
    echo "提示: 请优先确认以下两点：" | tee -a "$LOG_FILE"
    echo "  1. 在开发机已执行 scripts/prepare_sciworld_wheels.sh 并成功下载完整依赖树" | tee -a "$LOG_FILE"
    echo "  2. third_party/wheels_sciworld/ 中确实存在 .whl 文件后再重新上传 OSS" | tee -a "$LOG_FILE"
    echo "wheel 目录: $WHEEL_DIR" | tee -a "$LOG_FILE"
    echo "requirements: $RUNTIME_REQS" | tee -a "$LOG_FILE"
    exit 1
fi
echo "SciWorld 运行时依赖安装完成" | tee -a "$LOG_FILE"

configure_sciworld_java

echo "安装 agentenv-sciworld..." | tee -a "$LOG_FILE"
cd "$AGENTGYM_ROOT/agentenv-sciworld"
if ! python3 -m pip install --no-build-isolation -e . --no-deps -q 2>&1 | tee -a "$LOG_FILE"; then
    echo "错误: agentenv-sciworld 安装失败" | tee -a "$LOG_FILE"
    exit 1
fi
echo "agentenv-sciworld 安装完成" | tee -a "$LOG_FILE"

echo "启动 SciWorld 服务 (${SCIWORLD_SERVER})..." | tee -a "$LOG_FILE"
sciworld --host 0.0.0.0 --port ${SCIWORLD_PORT} > "$SCIWORLD_LOG" 2>&1 &
SCIWORLD_PID=$!
echo "SciWorld 进程 PID=$SCIWORLD_PID，日志: $SCIWORLD_LOG" | tee -a "$LOG_FILE"

# 等待服务 ready（最多 120 秒）
READY=false
for i in $(seq 1 60); do
    if curl -sf "${SCIWORLD_SERVER}/" > /dev/null 2>&1; then
        READY=true
        break
    fi
    echo "  等待 SciWorld 服务启动... ($i/60)" | tee -a "$LOG_FILE"
    sleep 2
done

if [ "$READY" != "true" ]; then
    echo "错误: SciWorld 环境服务启动超时（120秒），请查看 $SCIWORLD_LOG" | tee -a "$LOG_FILE"
    exit 1
fi
echo "SciWorld 环境服务已就绪 (${SCIWORLD_SERVER})" | tee -a "$LOG_FILE"
echo "============================================" | tee -a "$LOG_FILE"
...
echo "" | tee -a "$LOG_FILE"
echo "============================================" | tee -a "$LOG_FILE"
echo "SciWorld GRPO 训练 - Qwen3-1.7B" | tee -a "$LOG_FILE"
echo "============================================" | tee -a "$LOG_FILE"
...
EXPERIMENT_NAME="sciworld_grpo_${TIMESTAMP}"

python3 -m verl.trainer.main_ppo \
    --config-path="${CONFIG_ROOT}" \
    --config-name='sciworld_grpo_train' \
    ...
    trainer.project_name=sciworld_grpo \
