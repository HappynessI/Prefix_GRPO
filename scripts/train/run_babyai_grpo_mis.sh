...
# BabyAI GRPO+MIS training entrypoint. The base GRPO script owns environment
# setup and training; this wrapper only enables rollout correction defaults.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT=${PROJECT_ROOT:-"$(cd "${SCRIPT_DIR}/../.." && pwd)"}
BASE_SCRIPT="${SCRIPT_DIR}/run_babyai_grpo_train.sh"
...
export OUTPUT_DIR="${OUTPUT_DIR:-${OUTPUT_ROOT_DEFAULT}/babyai_grpo_mis}"
