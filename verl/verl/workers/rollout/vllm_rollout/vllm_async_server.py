...
def _resolve_effective_max_model_len(max_model_len: Optional[int], prompt_length: int, response_length: int) -> int:
    """Preserve explicit context-budget overrides for multi-turn rollouts.

    `prompt_length + response_length` is the minimum training tensor budget, but
    the live decoder prompt in agent environments can grow beyond that because it
    also includes intermediate user/tool observations. When callers explicitly
    set `rollout.max_model_len`, we should keep that value instead of shrinking
    it back to the training budget.
    """
    min_required_model_len = prompt_length + response_length
    if max_model_len is None:
        return min_required_model_len
    if max_model_len < min_required_model_len:
        logger.warning(
            "Configured rollout.max_model_len=%s is smaller than prompt_length + response_length=%s. "
            "This leaves no extra headroom for multi-turn observations and can cause generation to stop early.",
            max_model_len,
            min_required_model_len,
        )
...
class vLLMHttpServerBase:
    ...
    def __init__(
        ...
        self.config.max_model_len = _resolve_effective_max_model_len(
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        # Set up data parallel params.
        # This block must run regardless of expert_parallel_size, because vLLM needs
        # data_parallel_* to know how many EngineCoreProc workers to launch and how to
        # connect them. The `data_parallel_size_local` is derived from `gpus_per_node`
        # (which equals `len(self.workers) == self.world_size` in the default
        # 2-replica × 1-card layout) and tensor_model_parallel_size.
        assert self.gpus_per_node % self.config.tensor_model_parallel_size == 0, (
            f"gpus_per_node ({self.gpus_per_node}) should be divisible by "
        ...
        assert len(self.workers) == data_parallel_size_local * self.config.tensor_model_parallel_size, (
            ...
            f"data_parallel_size_local ({data_parallel_size_local}) * "
            f"tp_size ({self.config.tensor_model_parallel_size})"
        )
        ...
        if self.config.expert_parallel_size > 1:
            dp_args["enable_expert_parallel"] = True
        args.update(dp_args)
    ...
    async def generate(
        ...
        if len(prompt_ids) > self.config.max_model_len:
            raise ValueError(
                f"The decoder prompt (length {len(prompt_ids)}) is longer than rollout.max_model_len "
                f"({self.config.max_model_len}). Increase actor_rollout_ref.rollout.max_model_len to leave "
                "headroom for multi-turn user/tool observations."
            )
    ...
    async def wait_for_requests_to_drain(self):
        ...
        if wait_for_requests_to_drain is None:
            logger.warning("AsyncLLM does not support wait_for_requests_to_drain(); skipping drain step.")
            return
...
class vLLMReplica(RolloutReplica):
    ...
    async def launch_servers(self):
        ...
        assert len(self.workers) % self.nnodes == 0, (
            f"num workers ({len(self.workers)}) must be evenly divisible by nnodes ({self.nnodes})"
        )
        ...
        if self.config.data_parallel_size == 1:
            nnodes = 1
            # Each replica has `len(self.workers) == self.world_size` workers.
            # In the default 2-replica × 1-card layout (tp=1, dp=1, pp=1),
            # each replica has exactly 1 worker and this equals `self.world_size`.
            gpus_per_node = len(self.workers)
