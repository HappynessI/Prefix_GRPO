...
class vLLMAsyncRollout(BaseRollout):
    ...
    def _init_worker(self, all_kwargs: list[dict[str, Any]]):
        ...
        Mode split:
        - HYBRID:     PG was initialized by FSDP training (world_size=2). We skip
                      init entirely to preserve the NCCL communicator. vLLM uses the
                      existing PG; its world_group is [0,1] which is correct since
                      both ranks exist in this mode.
        - STANDALONE: PG was never initialized (fresh worker process). We init with
                      world_size=1 so vLLM's world_group is [0] and no rank-1 is waited.
        - NEVER call destroy_process_group(): it destroys the global NCCL PG singleton.
          In HYBRID mode the same PG is held by FSDP in rank-0's process; destroying
          it from any worker breaks the FSDP communicator -> NCCL abort on state_dict().
        """
        orig_rank = os.environ.get("RANK")
        orig_ws = os.environ.get("WORLD_SIZE")
        try:
            if torch.distributed.is_initialized():
                pass  # PG already exists; keep it as-is (HYBRID mode)
            else:
                os.environ["WORLD_SIZE"] = str(len(all_kwargs))
                os.environ["RANK"] = "0"
