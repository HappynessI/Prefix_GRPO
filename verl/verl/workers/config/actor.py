...
class ActorConfig(BaseConfig):
    ...
    prefix_clip_ratio: Optional[float] = None
    prefix_clip_ratio_low: Optional[float] = None
    prefix_clip_ratio_high: Optional[float] = None
    freeze_vision_tower: bool = False
    policy_loss: PolicyLossConfig = field(default_factory=PolicyLossConfig)
    clip_ratio_c: float = 3.0
    prefix_clip_ratio_c: Optional[float] = None
    ...
    def validate(self, n_gpus: int, train_batch_size: int, model_config: dict = None):
        ...
        if not self.use_dynamic_bsz:
            ...
            if dp_size <= 0:
                raise ValueError(f"Invalid dp_size computed from n_gpus={n_gpus}, sp_size={sp_size}")

            effective_local_ppo_mini_batch_size = self.ppo_mini_batch_size * self.rollout_n // dp_size
            if effective_local_ppo_mini_batch_size <= 0:
                raise ValueError(
                    "effective local ppo mini-batch size must be positive after rollout expansion: "
                    f"ppo_mini_batch_size={self.ppo_mini_batch_size}, rollout_n={self.rollout_n}, dp_size={dp_size}"
                )

            if self.ppo_micro_batch_size_per_gpu is not None:
                if effective_local_ppo_mini_batch_size % self.ppo_micro_batch_size_per_gpu != 0:
                    raise ValueError(
                        "effective local ppo mini-batch size "
                        f"({effective_local_ppo_mini_batch_size}) must be divisible by "
                        f"ppo_micro_batch_size_per_gpu ({self.ppo_micro_batch_size_per_gpu})"
                    )
                if effective_local_ppo_mini_batch_size // self.ppo_micro_batch_size_per_gpu <= 0:
                    raise ValueError(
                        "ppo_micro_batch_size_per_gpu "
                        f"({self.ppo_micro_batch_size_per_gpu}) must be <= effective local "
                        f"ppo mini-batch size ({effective_local_ppo_mini_batch_size})"
                    )
