...
class MultiTurnSFTDataset(Dataset):
    ...
    def __init__(self, parquet_files: str | list[str], tokenizer, config=None, max_samples: int = -1):
        ...
        self.default_enable_thinking = multiturn_config.get("default_enable_thinking", None)
        if isinstance(self.default_enable_thinking, np.bool_):
            self.default_enable_thinking = bool(self.default_enable_thinking)
        if self.default_enable_thinking is not None and not isinstance(self.default_enable_thinking, bool):
            raise TypeError(
                f"default_enable_thinking should be bool or None, got {type(self.default_enable_thinking)}"
            )
    ...
    def _read_files_and_process(self):
        ...
        elif self.default_enable_thinking is not None:
            self.enable_thinking = [self.default_enable_thinking] * len(self.dataframe)
        else:
