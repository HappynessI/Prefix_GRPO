...
def _is_legacy_hf_list_feature_error(exception: Exception) -> bool:
    seen = set()
    current: BaseException | None = exception
    while current is not None and id(current) not in seen:
        if "Feature type 'List' not found" in str(current):
            return True
        seen.add(id(current))
    ...
    def _read_files_and_tokenize(self):
        ...
        for parquet_file in self.data_files:
            ...
            try:
                ...
                if not _is_legacy_hf_list_feature_error(exc):
                    raise

                logger.warning(
                    "datasets.load_dataset failed on %s with legacy List feature metadata; "
                    "falling back to pandas parquet reader.",
                    parquet_file,
                )
                import pandas as pd
