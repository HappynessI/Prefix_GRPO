...
def _is_babyai_initial_placeholder(messages: list[dict[str, Any]], interaction_kwargs: dict[str, Any]) -> bool:
    if interaction_kwargs.get("name") != "babyai" or len(messages) != 2:
        return False
    if messages[0].get("role") != "system" or messages[1].get("role") != "user":
        return False
    return str(messages[1].get("content", "")).strip() == BABYAI_INITIAL_PROMPT_PLACEHOLDER
...
def _needs_initial_observation(messages: list[dict[str, Any]], interaction_kwargs: dict[str, Any]) -> bool:
    ...
    if len(messages) == 1 and messages[0].get("role") == "system":
        return True
    return _is_babyai_initial_placeholder(messages, interaction_kwargs)
...
def _apply_initial_observation(
    ...
    if _is_babyai_initial_placeholder(messages, interaction_kwargs):
