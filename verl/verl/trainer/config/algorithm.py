...
class AlgoConfig(BaseConfig):
    ...
    # Prefix loss structure:
    # - "split": continuation loss + weighted prefix loss
    # - "joint": concatenate valid prefix tokens with continuation tokens into one PPO objective
    prefix_loss_mode: str = "split"
    # Prefix advantage options:
    # - "cont_mean": masked mean of continuation advantages
    # - "cont_mean_abs": absolute value of masked mean continuation advantages
    # - "cont_mean_pos": positive part of masked mean continuation advantages
    # - "constant": fixed positive scalar configured by prefix_advantage_constant
    # - "success_gate_constant": constant only for successful rollouts
    # - "family_lift*": raw-vs-prefix success lift within the same sample_uid family
    prefix_advantage_mode: str = "cont_mean"
    # Constant prefix advantage used when prefix_advantage_mode == "constant"
    prefix_advantage_constant: float = 1.0
    # Weights used by family-lift prefix advantage modes.
    prefix_cont_adv_weight: float = 1.0
    prefix_family_lift_weight: float = 1.0
    prefix_family_lift_clip: float = 1.0

    # Optional generic auxiliary behavior-cloning loss on logged continuation assistant actions.
    use_bc_aux: bool = False
    bc_weight: float = 0.0
    bc_source: str = "prefix"
    bc_max_length: Optional[int] = None

    # Optional generic logged teacher-demo trajectories.
    use_teacher_demo: bool = False
    teacher_demo_weight: float = 1.0
    teacher_demo_labels: list[str] | str = field(default_factory=lambda: ["teacher_demo", "demo"])
    teacher_demo_repeat_to_rollout_n: bool = True
    teacher_demo_skip_overlong: bool = False

    # Backward-compatible TextCraft-prefixed aliases.
    use_textcraft_bc_aux: bool = False
    textcraft_bc_weight: float = 0.0
    textcraft_bc_source: str = "prefix"
    textcraft_bc_max_length: Optional[int] = None

    use_textcraft_teacher_demo: bool = False
    textcraft_teacher_demo_weight: float = 1.0
    textcraft_teacher_demo_labels: list[str] | str = field(default_factory=lambda: ["teacher_demo", "demo"])
    textcraft_teacher_demo_repeat_to_rollout_n: bool = True
    textcraft_teacher_demo_skip_overlong: bool = False
