...
def _compute_group_reward_metrics(batch: DataProto, sequence_score: torch.Tensor) -> dict[str, Any]:
    """
    Compute prompt-group reward diagnostics for GRPO-style repeated rollouts.

    The training batch repeats each prompt `rollout.n` times and preserves a stable `uid`
    across repeats. This helper groups responses by uid and reports whether a prompt's
    entire rollout group received zero reward.
    """
    uid_values = None
    if "uid" in batch.non_tensor_batch:
        uid_values = batch.non_tensor_batch["uid"]
    elif "uid" in batch.batch:
        uid_values = batch.batch["uid"]

    if uid_values is None:
        return {}

    uid_array = np.asarray(uid_values, dtype=object).reshape(-1)
    score_array = sequence_score.detach().float().cpu().numpy().reshape(-1)

    if uid_array.shape[0] != score_array.shape[0]:
        return {}

    uid2scores = defaultdict(list)
    for uid, score in zip(uid_array, score_array, strict=True):
        uid2scores[str(uid)].append(float(score))

    if not uid2scores:
        return {}

    eps = 1e-8
    group_sizes = np.array([len(scores) for scores in uid2scores.values()], dtype=np.int32)
    all_zero_mask = np.array([all(abs(score) <= eps for score in scores) for scores in uid2scores.values()])
    any_positive_mask = np.array([any(score > eps for score in scores) for scores in uid2scores.values()])
    positive_counts = np.array([sum(score > eps for score in scores) for scores in uid2scores.values()], dtype=np.int32)
    group_reward_sums = np.array([sum(scores) for scores in uid2scores.values()], dtype=np.float32)

    return {
        "group/reward_all_zero_rate": float(all_zero_mask.mean()),
        "group/reward_at_least_one_positive_rate": float(any_positive_mask.mean()),
        "group/reward_positive_responses_per_group_mean": float(positive_counts.mean()),
        "group/reward_positive_responses_per_group_max": int(positive_counts.max()),
        "group/reward_sum_per_group_mean": float(group_reward_sums.mean()),
        "group/size_mean": float(group_sizes.mean()),
        "group/size_min": int(group_sizes.min()),
        "group/size_max": int(group_sizes.max()),
    }


def _numeric_non_tensor(values: Any) -> np.ndarray | None:
    try:
        ...
        return None
    if converted.size == 0 or np.all(np.isnan(converted)):
        return None
    return converted
...
def _compute_textcraft_rollout_metrics(batch: DataProto) -> dict[str, Any]:
    """Aggregate TextCraft episode-level diagnostics emitted by TextCraftInteraction."""
    numeric: dict[str, np.ndarray] = {}
    for key, values in batch.non_tensor_batch.items():
        if not isinstance(key, str) or not key.startswith("textcraft/"):
            continue
        arr = _numeric_non_tensor(values)
        if arr is not None:
            numeric[key] = arr
    ...
    metrics: dict[str, Any] = {key: float(np.nanmean(arr)) for key, arr in numeric.items()}

    success = numeric.get("textcraft/success")
    if success is None:
        return metrics

    group_masks = {
        "raw": numeric.get("textcraft/is_raw"),
        "prefix": numeric.get("textcraft/is_prefix"),
        "prefix_non_terminal": numeric.get("textcraft/is_prefix_non_terminal"),
        "prefix_terminal_like": numeric.get("textcraft/is_prefix_terminal_like"),
    }
    for group_name, mask_values in group_masks.items():
        if mask_values is None:
            continue
        group_mean = _masked_mean(success, mask_values > 0.5)
        if group_mean is not None:
            metrics[f"textcraft/success/{group_name}_mean"] = group_mean
...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    Returns:
        A dictionary of metrics including:
            ...
            - assistant_response_length/mean, max, min: Statistics about assistant-generated response lengths
    ...
    assistant_response_length = response_mask.sum(-1).float()
    ...
    metrics = {
        ...
        **(
        ...
        .detach()
        .item(),
        # assistant-only response length
        # Counts LLM-generated tokens only; excludes environment/tool observation tokens in multi-turn rollouts.
        "assistant_response_length/mean": torch.mean(assistant_response_length).detach().item(),
        "assistant_response_length/max": torch.max(assistant_response_length).detach().item(),
        "assistant_response_length/min": torch.min(assistant_response_length).detach().item(),
    ...
    metrics.update(_compute_group_reward_metrics(batch=batch, sequence_score=sequence_score))
    metrics.update(_compute_textcraft_rollout_metrics(batch=batch))
